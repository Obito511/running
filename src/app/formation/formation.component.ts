...
export class FormationComponent implements OnInit {
  ...
  selectedB:any="Tous les formations"
...
return url.substring(url.lastIndexOf('/'),url.length)
 ...
 if(this.selectedA=="Tous les années"){
  ...
  getMesFormation(event:any){
    
    return this.http.get(this.baseURL+"/mesformation",{headers:this.header})
    .subscribe({
      next: (res) => {
       this.response=res;
      this.response.reverse()
      if(this.selectedB =="Tous les formations"){
        this.response=this.backup
       }
      },
  error: (err) => console.log(err),
